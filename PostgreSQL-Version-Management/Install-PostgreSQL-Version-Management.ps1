...
$ModuleFiles = @(
	@{
		...
		Content = @(
			...
			"  CompanyName = ''",
			"",
			"  Copyright = '(c) 2023-2024 devizer.'",
			"",
			"  Description = @`"",
			"Postgre SQL Server Setup.",
			"The intended use of this project is for Continuous Integration (CI) scenarios, where:",
			"     - A Postgre SQL needs to be installed without user interaction.",
			"     - A Postgre SQL installation doesn't need to persist across multiple CI runs.",
			"`"@",
			"",
			...
			"        ",
			"        Tags = @(",
			"          `"Windows`", `"Postgre SQL Server`", `"SQL`"",
			"        )",
			"",
   ...
   ,
	@{
		FileName = 'PostgreSQL-Version-Management\PostgreSQL-Version-Management.psm1';
		Content = @(
			"# Include Directive: [ src\Synopsis.ps1 ]",
			"# Include File: [\Postgres-Version-Manager.PS1Project\src\Synopsis.ps1]",
			"<#",
			" .Synopsis",
			"  Postgre SQL Server Setup.",
			"  The intended use of this project is for Continuous Integration (CI) scenarios, where:",
			...
			"  log_rotation_size = 0",
			"  log_rotation_age = 0",
			"  log_truncate_on_rotation = off$([char]9)$([char]9)",
			"  log_statement = 'all'",
			"  log_duration = on",
			"",
			"  max_connections = 200",
			...
			"",
			"",
			"Export-ModuleMember -Function Setup-PostgreSQL-Server",
			"Export-ModuleMember -Function Install-VC-Redist-for-Postgres-On-Windows",
			"Export-ModuleMember -Function Get-Available-PostgreSQL-Versions",
			"",
			"Export-ModuleMember -Function Get-Postgres-Download-Links",
			...
			"Export-ModuleMember -Function Demo-Test-of-Is-Vc-Runtime-Installed",
			"Export-ModuleMember -Function Demo-Test-of-Platform-Info",
			...
			"Export-ModuleMember -Function Get-ANSI-Colors",
			...
			"Export-ModuleMember -Function Get-CPU-Architecture-Suffix-for-Windows-Implementation",
			"Export-ModuleMember -Function Get-Cpu-Name",
			"Export-ModuleMember -Function Get-Cpu-Name-Implementation",
			...
			"Export-ModuleMember -Function Update-Lazy-Aggregator",
			"Export-ModuleMember -Function Write-All-Text",
			"",
			"Export-ModuleMember -Variable `"```$AVAILABLE_POSTGRESQL_VERSIONS`"",
			"Export-ModuleMember -Variable `"```$KNOWN_POSTGRESQL_FULL_DIRECT_LINKS`""
		);
	}
);
